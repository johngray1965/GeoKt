...
class AndroidMatrixTest {
    ...
    fun mapRectTwoArgsBehavesLikePlatformMatrix() {
        ...
        val srcRect2 = KtRectF(0f, 0f, 100f, 200f)
        ...
        mutableMatrix.mapRect(customDstRect, srcRect2)
        result.mapRect(customDstRect2, srcRect2)
...
//    }
    ...
    @Test
    fun setPolyToPoly_0Points_ResetsToIdentity() {
        mutableMatrix.setScale(2f, 2f)
        platformMatrix.setScale(2f, 2f)
        val src = floatArrayOf()
        val dst = floatArrayOf()

        val mutableResult = mutableMatrix.setPolyToPoly(src, 0, dst, 0, 0)
        val platformResult = platformMatrix.setPolyToPoly(src, 0, dst, 0, 0)
        val immutableResult = pdfMatrix.setPolyToPoly(src, 0, dst, 0, 0)

        assertThat(mutableResult).isEqualTo(platformResult)
        assertThat(mutableMatrix.isIdentity()).isTrue()
        assertThat(immutableResult!!.isIdentity()).isTrue()
        assertMatrixValuesEqual(mutableMatrix, platformMatrix)
        assertMatrixValuesEqual(immutableResult, platformMatrix)
    }

    @Test
    fun setPolyToPoly_1Point_IsTranslation() {
        val src = floatArrayOf(10f, 20f)
        val dst = floatArrayOf(110f, 120f)

        val mutableResult = mutableMatrix.setPolyToPoly(src, 0, dst, 0, 1)
        val platformResult = platformMatrix.setPolyToPoly(src, 0, dst, 0, 1)
        val immutableResult = pdfMatrix.setPolyToPoly(src, 0, dst, 0, 1)

        assertThat(mutableResult).isEqualTo(platformResult)
        assertMatrixValuesEqual(mutableMatrix, platformMatrix)
        assertMatrixValuesEqual(immutableResult!!, platformMatrix)

        val expected = AndroidPlatformMatrix().apply { setTranslate(100f, 100f) }
        assertMatrixValuesEqual(mutableMatrix, expected)
    }

    @Test
    fun setPolyToPoly_2Points_IsScaleRotate() {
        val src = floatArrayOf(0f, 0f, 10f, 0f)
        val dst = floatArrayOf(10f, 10f, 20f, 20f)

        val mutableResult = mutableMatrix.setPolyToPoly(src, 0, dst, 0, 2)
        val platformResult = platformMatrix.setPolyToPoly(src, 0, dst, 0, 2)
        val immutableResult = pdfMatrix.setPolyToPoly(src, 0, dst, 0, 2)
    ...
    @Test
    fun setPolyToPoly_4Points_IsPerspective() {
        val src = floatArrayOf(0f, 0f, 10f, 0f, 10f, 10f, 0f, 10f)
        val dst = floatArrayOf(5f, 5f, 15f, 0f, 20f, 15f, 0f, 20f)

        val mutableResult = mutableMatrix.setPolyToPoly(src, 0, dst, 0, 4)
        val platformResult = platformMatrix.setPolyToPoly(src, 0, dst, 0, 4)
        val immutableResult = pdfMatrix.setPolyToPoly(src, 0, dst, 0, 4)

        println("mutableMatrix: $mutableMatrix")
        println("platformMatrix: $platformMatrix")
        println("immutableResult: $immutableResult")
    ...
    private fun assertMatrixValuesEqual(
        ...
        assertFloatArraysEqual(platformValues, customValues.map { it.toFloat()}.toFloatArray(), delta)
    }
